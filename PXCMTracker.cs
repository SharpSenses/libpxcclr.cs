...
public class PXCMTracker : PXCMBase {
    ...
    [DllImport("libpxccpp2c", CharSet = CharSet.Unicode)]
    internal static extern pxcmStatus PXCMTracker_Set2DTrackFromFile(IntPtr instance, string filename, out int cosID,
        float widthMM, float heightMM, float qualityThreshold, [MarshalAs(UnmanagedType.Bool)] bool extensible);

    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMTracker_Set2DTrackFromImage(IntPtr instance, IntPtr image, out int cosID,
        float widthMM, float heightMM, float qualityThreshold, [MarshalAs(UnmanagedType.Bool)] bool extensible);

    [DllImport("libpxccpp2c", CharSet = CharSet.Unicode)]
    internal static extern pxcmStatus PXCMTracker_Set3DTrack(IntPtr instance, string filename, out int firstCosID,
        out int lastCosID, [MarshalAs(UnmanagedType.Bool)] bool extensible);
    ...
    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMTracker_Set3DInstantTrack(IntPtr instance,
        [MarshalAs(UnmanagedType.Bool)] bool egoMotion, int framesToSkip);
    ...
    internal static pxcmStatus QueryAllTrackingValuesINT(IntPtr instance, out TrackingValues[] trackingValues) {
        var length = PXCMTracker_QueryNumberTrackingValues(instance);
        var num = Marshal.AllocHGlobal(Marshal.SizeOf(typeof (TrackingValues))*length);
        var pxcmStatus = PXCMTracker_QueryAllTrackingValues(instance, num);
        if (pxcmStatus >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            ...
            for (var index = 0; index < length; ++index) {
                ...
                Marshal.PtrToStructure(new IntPtr(num.ToInt64() + Marshal.SizeOf(typeof (TrackingValues))*index),
    ...
    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMTracker_QueryTrackingValues(IntPtr instance, int cosID,
        [Out] TrackingValues trackingValues);
    ...
    public pxcmStatus Set2DTrackFromFile(string filename, out int cosID, float widthMM, float heightMM,
        float qualityThreshold, bool extensible) {
        return PXCMTracker_Set2DTrackFromFile(instance, filename, out cosID, widthMM, heightMM, qualityThreshold,
            extensible);
    }
    ...
    public pxcmStatus Set2DTrackFromImage(PXCMImage image, out int cosID, float widthMM, float heightMM,
        float qualityThreshold, bool extensible) {
        if (image != null) {
            return PXCMTracker_Set2DTrackFromImage(instance, image.instance, out cosID, widthMM, heightMM,
                qualityThreshold, extensible);
        }
        cosID = 0;
    ...
    public class TrackingValues {
        ...
        public int cosID;
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)] public string targetName;
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)] public string additionalValues;
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)] public string sensor;
        public PXCMPointF32 translationImage;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 30)] internal int[] reserved;
