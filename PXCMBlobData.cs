...
public class PXCMBlobData : PXCMBase
{
  ...
  [DllImport("libpxccpp2c")]
  internal static extern pxcmStatus PXCMBlobData_QueryBlobByAccessOrder(IntPtr instance, int index, AccessOrderType accessOrder, out IntPtr blobData);
  ...
  {
    IntPtr blobData1;
    pxcmStatus pxcmStatus = PXCMBlobData_QueryBlobByAccessOrder(instance, index, accessOrderType, out blobData1);
  ...
  public class IBlob
  {
    ...
    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMBlobData_IBlob_QueryContourPoints(IntPtr instsance, int index, int max, [Out] PXCMPointI32[] contour);

    internal static pxcmStatus QueryContourDataINT(IntPtr instance, int index, out PXCMPointI32[] contour)
    {
      int max = PXCMBlobData_IBlob_QueryContourSize(instance, index);
      if (max > 0)
      {
    ...
    {
      IntPtr image1;
      pxcmStatus pxcmStatus = PXCMBlobData_IBlob_QuerySegmentationImage(instance, out image1);
