...
public class PXCMImage : PXCMBase
{
  ...
  {
    ImageDataNative imageDataNative = new ImageDataNative();
    imageDataNative.native = Marshal.AllocHGlobal(Marshal.SizeOf(typeof (ImageDataNative)));
    data = imageDataNative;
    pxcmStatus pxcmStatus = PXCMImage_AcquireAccess(instance, access, format, options, imageDataNative.native);
    if (pxcmStatus >= pxcmStatus.PXCM_STATUS_NO_ERROR)
    {
      IntPtr num = imageDataNative.native;
  ...
  {
    ImageDataNative imageDataNative = data as ImageDataNative;
    if (imageDataNative == null || imageDataNative.native == IntPtr.Zero)
      return pxcmStatus.PXCM_STATUS_HANDLE_INVALID;
    pxcmStatus status = PXCMImage_ReleaseAccess(instance, imageDataNative.native);
    Marshal.FreeHGlobal(imageDataNative.native);
    imageDataNative.native = IntPtr.Zero;
    return status;
  }
  ...
  {
    IntPtr[] numArray = new IntPtr[images.Length];
  ...
  public class ImageData
  {
    ...
    public Bitmap ToBitmap(int index, int width, int height)
    {
      ...
      {
        case PixelFormat.PIXEL_FORMAT_RGB32:
          bitmap = new Bitmap(width, height, pitches[index], System.Drawing.Imaging.PixelFormat.Format32bppRgb, planes[0]);
          break;
        case PixelFormat.PIXEL_FORMAT_RGB24:
    ...
    public Bitmap ToBitmap(int index, Bitmap bitmap)
    {
      ...
      {
        case PixelFormat.PIXEL_FORMAT_RGB32:
          BitmapData bitmapdata1 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
          PXCMImage_ImageData_Copy(planes[index], pitches[index], bitmapdata1.Scan0, bitmapdata1.Stride, bitmap.Width * 4, bitmap.Height);
          bitmap.UnlockBits(bitmapdata1);
          break;
        case PixelFormat.PIXEL_FORMAT_RGB24:
          BitmapData bitmapdata2 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
          PXCMImage_ImageData_Copy(planes[index], pitches[index], bitmapdata2.Scan0, bitmapdata2.Stride, bitmap.Width * 3, bitmap.Height);
          bitmap.UnlockBits(bitmapdata2);
          break;
        case PixelFormat.PIXEL_FORMAT_DEPTH:
        case PixelFormat.PIXEL_FORMAT_DEPTH_RAW:
          BitmapData bitmapdata3 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, System.Drawing.Imaging.PixelFormat.Format16bppGrayScale);
          PXCMImage_ImageData_Copy(planes[index], pitches[index], bitmapdata3.Scan0, bitmapdata3.Stride, bitmap.Width * 2, bitmap.Height);
    ...
    [DllImport("libpxccpp2c")]
    private static extern void PXCMImage_ImageData_Copy(IntPtr src, int spitch, IntPtr dst, int dpitch, int width, int height);
    ...
    public ushort[] ToUShortArray(int index, ushort[] dest)
    {
      ...
      int num = pitches[index] / 2;
      PXCMImage_ImageData_Copy(planes[index], pitches[index], gcHandle.AddrOfPinnedObject(), pitches[index], num * 2, dest.Length / num);
      gcHandle.Free();
      return dest;
    }
    ...
    public void FromUShortArray(int index, ushort[] src)
    {
      ...
      int num = pitches[index] / 2;
      PXCMImage_ImageData_Copy(gcHandle.AddrOfPinnedObject(), pitches[index], planes[index], pitches[index], num * 2, src.Length / num);
    }
    ...
    public WriteableBitmap ToWritableBitmap(int index, int width, int height, double dpiX, double dpiY)
    {
      ...
      switch (format)
      {
        case PixelFormat.PIXEL_FORMAT_RGB32:
          ...
          writeableBitmap.WritePixels(new Int32Rect(0, 0, width, height), planes[index], pitches[index] * height, pitches[index]);
          break;
        case PixelFormat.PIXEL_FORMAT_RGB24:
