...
public class PXCMContourExtractor : PXCMBase
{
  ...
  [DllImport("libpxccpp2c")]
  private static extern pxcmStatus PXCMContourExtractor_QueryContourData(IntPtr instsance, int index, int max, [Out] PXCMPointI32[] contour);

  internal static pxcmStatus QueryContourDataINT(IntPtr instance, int index, out PXCMPointI32[] contour)
  {
    int max = PXCMContourExtractor_QueryContourSize(instance, index);
    if (max > 0)
    {
