...
public class PXCMSession : PXCMBase {
    ...
    public PXCMLoggingService CreateLogger(string loggerName) {
        ...
        if (CreateImpl(out tt) >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            var num = (int) tt.SetLoggerName(loggerName);
        }
        else {
    ...
    public ImplVersion QueryVersion() {
        var version = new ImplVersion();
        var num = (int) PXCMSession_QueryVersion(instance, version);
        return version;
    }

    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMSession_QueryImpl(IntPtr session, ImplDesc templat, int idx,
        [Out] ImplDesc desc);
    ...
    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMSession_CreateImpl(IntPtr session, ImplDesc desc, int iuid, int cuid,
        out IntPtr instance);
    ...
    [DllImport("libpxccpp2c")]
    internal static extern IntPtr PXCMSession_CreateImage(IntPtr session, PXCMImage.ImageInfo info,
        PXCMImage.ImageData data);

    [DllImport("libpxccpp2c")]
    internal static extern IntPtr PXCMSession_CreateAudio(IntPtr session, PXCMAudio.AudioInfo info,
    ...
    public pxcmStatus CreateImpl(ImplDesc desc, int iuid, int cuid, out PXCMBase impl) {
        IntPtr instance;
        var impl1 = PXCMSession_CreateImpl(this.instance, desc, iuid, cuid, out instance);
        impl = null;
    ...
    public pxcmStatus CreateImpl<TT>(ImplDesc desc, int iuid, out TT tt) where TT : PXCMBase {
        PXCMBase impl1;
        var impl2 = CreateImpl(desc, iuid, Type2CUID[typeof (TT)], out impl1);
    ...
    public pxcmStatus CreateImpl<TT>(ImplDesc desc, out TT tt) where TT : PXCMBase {
        PXCMBase instance;
        var impl = CreateImpl(desc, Type2CUID[typeof (TT)], out instance);
    ...
    public pxcmStatus CreateImpl<TT>(int iuid, out TT tt) where TT : PXCMBase {
        PXCMBase instance;
        var impl = CreateImpl(iuid, Type2CUID[typeof (TT)], out instance);
        tt = impl >= pxcmStatus.PXCM_STATUS_NO_ERROR ? (TT) instance : default(TT);
        return impl;
    }
    ...
    public pxcmStatus CreateImpl<TT>(out TT tt) where TT : PXCMBase {
        PXCMBase instance;
        var impl = CreateImpl(Type2CUID[typeof (TT)], out instance);
        tt = impl >= pxcmStatus.PXCM_STATUS_NO_ERROR ? (TT) instance : default(TT);
        return impl;
    }
    ...
    public PXCMImage CreateImage(PXCMImage.ImageInfo info, PXCMImage.ImageData data) {
        var image = PXCMSession_CreateImage(instance, info, data);
    ...
    public PXCMAudio CreateAudio(PXCMAudio.AudioInfo info, PXCMAudio.AudioData data) {
        var audio = PXCMSession_CreateAudio(instance, info, data);
    ...
    public static PXCMSession CreateInstance() {
        var instance = PXCMSession_Create();
        var pxcmSession = instance != IntPtr.Zero ? new PXCMSession(instance, true) : null;
        if (pxcmSession == null) {
            return null;
        }
        try {
    ...
    public class ImplDesc {
        ...
        public int merit;
        public int vendor;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 4)] public int[] cuids;
        [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)] public string friendlyName;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 12)] internal int[] reserved;
