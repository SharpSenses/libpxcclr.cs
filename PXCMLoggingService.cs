...
public class PXCMLoggingService : PXCMBase {
    ...
    [DllImport("libpxccpp2c", CharSet = CharSet.Unicode)]
    internal static extern pxcmStatus PXCMLoggingService_Configure(IntPtr instance, ConfigMode configMode,
        StringBuilder config, int fileWatchDelay);
    ...
    internal static extern void PXCMLoggingService_Log(IntPtr instance, Level level, StringBuilder message,
        StringBuilder fileName, int lineNumber, StringBuilder functionName);

    [DllImport("libpxccpp2c", CharSet = CharSet.Unicode)]
    internal static extern void PXCMLoggingService_LogW(IntPtr instance, Level level, StringBuilder message,
        StringBuilder fileName, int lineNumber, StringBuilder functionName);
    ...
    public pxcmStatus Configure(ConfigMode configMode, string config, int fileWatchDelay) {
        var config1 = new StringBuilder(config);
    ...
    public void Log(Level level, string message, string fileName, int lineNumber, string functionName) {
        var message1 = new StringBuilder(message);
        var fileName1 = new StringBuilder(fileName);
        var functionName1 = new StringBuilder(functionName);
    ...
    public void TaskBegin(Level level, string taskName) {
        var taskName1 = new StringBuilder(taskName);
