...
public class PXCMTouchlessController : PXCMBase {
    ...
    private static extern pxcmStatus PXCMTouchlessController_AddGestureActionMapping(IntPtr instance, string gestureName,
        Action action, IntPtr actionHandler);
    ...
    [DllImport("libpxccpp2c")]
    private static extern pxcmStatus PXCMTouchlessController_SetPointerSensitivity(IntPtr instance,
    ...
    internal static pxcmStatus SubscribeEventINT(IntPtr instance, OnFiredUXEventDelegate uxEventHandler,
        out object proxy) {
        var uxEventHandlerDir = new UXEventHandlerDIR(uxEventHandler);
        var pxcmStatus = PXCMTouchlessController_SubscribeEvent(instance, uxEventHandlerDir.uDIR);
    ...
    internal static pxcmStatus UnsubscribeEventINT(IntPtr instance, object proxy) {
        var uxEventHandlerDir = (UXEventHandlerDIR) proxy;
        var pxcmStatus = PXCMTouchlessController_UnsubscribeEvent(instance, uxEventHandlerDir.uDIR);
        uxEventHandlerDir.Dispose();
        return pxcmStatus;
    }

    internal static pxcmStatus SubscribeAlertINT(IntPtr instance, OnFiredAlertDelegate alertHandler, out object proxy) {
        var alertHandlerDir = new AlertHandlerDIR(alertHandler);
        var pxcmStatus = PXCMTouchlessController_SubscribeAlert(instance, alertHandlerDir.uDIR);
        if (pxcmStatus < pxcmStatus.PXCM_STATUS_NO_ERROR) {
    ...
    internal static pxcmStatus UnsubscribeAlertINT(IntPtr instance, object proxy) {
        ...
        var pxcmStatus = PXCMTouchlessController_UnsubscribeAlert(instance, alertHandlerDir.uDIR);
        alertHandlerDir.Dispose();
        return pxcmStatus;
    }

    internal static pxcmStatus AddGestureActionMappingINT(IntPtr instance, string gestureName, Action action,
        OnFiredActionDelegate actionHandler, out object proxy) {
        var actionHandlerDir = new ActionHandlerDIR(actionHandler);
        var pxcmStatus = PXCMTouchlessController_AddGestureActionMapping(instance, gestureName, action,
            actionHandlerDir.uDIR);
        if (pxcmStatus < pxcmStatus.PXCM_STATUS_NO_ERROR) {
    ...
    public pxcmStatus SubscribeEvent(OnFiredUXEventDelegate uxEventHandler) {
        ...
        object proxy;
        var status = SubscribeEventINT(instance, uxEventHandler, out proxy);
    ...
    public pxcmStatus UnsubscribeEvent(OnFiredUXEventDelegate uxEventHandler) {
        ...
        lock (maps.cs) {
            ...
            var local_1 = UnsubscribeEventINT(instance, local_0);
    ...
    public pxcmStatus SubscribeAlert(OnFiredAlertDelegate alertHandler) {
        ...
        object proxy;
        var status = SubscribeAlertINT(instance, alertHandler, out proxy);
        if (status >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            lock (maps.cs)
    ...
    public pxcmStatus AddGestureActionMapping(string gestureName, Action action, OnFiredActionDelegate actionHandler) {
        ...
        object proxy;
        var pxcmStatus = AddGestureActionMappingINT(instance, gestureName, action, actionHandler, out proxy);
        if (pxcmStatus >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            lock (maps.cs)
    ...
    public pxcmStatus ClearAllGestureActionMappings() {
        var pxcmStatus = PXCMTouchlessController_ClearAllGestureActionMappings(instance);
        lock (maps.cs)
            maps.actions.Clear();
        return pxcmStatus;
    }
    ...
    public class ProfileInfo {
        ...
        public enum Configuration {
            ...
            Configuration_Disable_On_Mouse_Move = 2048
        }
    }
