...
public class PXCMMetadata : PXCMBase
{
  ...
  internal static pxcmStatus AttachBufferINT(IntPtr md, int id, byte[] buffer)
  {
    ...
    pxcmStatus pxcmStatus = PXCMMetadata_AttachBuffer(md, id, gcHandle.AddrOfPinnedObject(), buffer.Length);
    gcHandle.Free();
    return pxcmStatus;
  }
  ...
  {
    int size = PXCMMetadata_QueryBufferSize(md, id);
    if (size == 0)
    {
      buffer = null;
  ...
  {
    IntPtr instance;
    pxcmStatus serializable = PXCMMetadata_CreateSerializable(this.instance, id, cuid, out instance);
    slz = null;
  ...
  {
    PXCMBase slz1;
    pxcmStatus serializable = CreateSerializable(id, Type2CUID[typeof (TT)], out slz1);
