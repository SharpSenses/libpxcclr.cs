...
public class PXCMCaptureManager : PXCMBase {
    ...
    [DllImport("libpxccpp2c")]
    internal static extern void PXCMCaptureManager_FilterByStreamProfiles(IntPtr cutil,
    ...
    public pxcmStatus RequestStreams(int mid, PXCMVideoModule.DataDesc inputs) {
        var objectPair = new ObjectPair(inputs);
        descriptors.Add(objectPair);
        var pxcmStatus = PXCMCaptureManager_RequestStreams(instance, mid, objectPair.unmanaged);
    ...
    public pxcmStatus LocateStreams(Handler handler) {
        var handlerDir = new HandlerDIR(handler);
        var pxcmStatus = PXCMCaptureManager_LocateStreams(instance, handlerDir.dirUnmanaged);
        if (pxcmStatus >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            foreach (var objectPair in descriptors) {
    ...
    [DllImport("libpxccpp2c")]
    internal static extern void PXCMCaptureManager_QueryImageSize(IntPtr putil, PXCMCapture.StreamType type,
        ref PXCMSizeI32 size);

    public PXCMSizeI32 QueryImageSize(PXCMCapture.StreamType type) {
        var size = new PXCMSizeI32();
        PXCMCaptureManager_QueryImageSize(instance, type, ref size);
        return size;
    }

    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMCaptureManager_ReadModuleStreamsAsync(IntPtr cutil, int mid,
        [Out] IntPtr[] images, out IntPtr sp);

    [DllImport("libpxccpp2c", CharSet = CharSet.Unicode)]
    internal static extern pxcmStatus PXCMCaptureManager_SetFileName(IntPtr cutil, string file,
        [MarshalAs(UnmanagedType.Bool)] bool record);
    ...
    [DllImport("libpxccpp2c")]
    internal static extern void PXCMCaptureManager_SetRealtime(IntPtr cutil,
        [MarshalAs(UnmanagedType.Bool)] bool realtime);
    ...
    public void FilterByDeviceInfo(string name, string did, int didx) {
        var dinfo = new PXCMCapture.DeviceInfo();
        if (name != null) {
            dinfo.name = name;
        }
        if (did != null) {
    ...
    public void FilterByStreamProfiles(PXCMCapture.StreamType type, int width, int height, float fps) {
        var profiles = new PXCMCapture.Device.StreamProfileSet();
    ...
    public PXCMCapture QueryCapture() {
        var instance = PXCMCaptureManager_QueryCapture(this.instance);
    ...
    public PXCMCapture.Device QueryDevice() {
        var instance = PXCMCaptureManager_QueryDevice(this.instance);
        if (instance == IntPtr.Zero) {
            return null;
        }
    ...
    public pxcmStatus ReadModuleStreamsAsync(int mid, PXCMCapture.Sample sample, out PXCMSyncPoint sp) {
        IntPtr sp1;
        var pxcmStatus = PXCMCaptureManager_ReadModuleStreamsAsync(instance, mid, sample.images, out sp1);
    ...
    internal class HandlerDIR : IDisposable {
        ...
        private pxcmStatus OnCreateDevice(PXCMSession.ImplDesc desc, IntPtr device) {
            var device1 = new PXCMCapture.Device(device, false);
