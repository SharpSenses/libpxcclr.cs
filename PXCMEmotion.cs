...
public class PXCMEmotion : PXCMBase {
    ...
    [DllImport("libpxccpp2c")]
    internal static extern pxcmStatus PXCMEmotion_QueryEmotionData(IntPtr emotion, int fid, Emotion eid,
        [Out] EmotionData data);
    ...
    internal static pxcmStatus QueryAllEmotionDataINT(IntPtr emotion, int fid, out EmotionData[] data) {
        var length = PXCMEmotion_QueryEmotionSize(emotion);
        var num = Marshal.AllocHGlobal(Marshal.SizeOf(typeof (EmotionData))*length);
        var pxcmStatus = PXCMEmotion_QueryAllEmotionData(emotion, fid, num);
        if (pxcmStatus >= pxcmStatus.PXCM_STATUS_NO_ERROR) {
            data = new EmotionData[length];
            for (var index = 0; index < data.Length; ++index) {
                data[index] = new EmotionData();
                Marshal.PtrToStructure(new IntPtr(num.ToInt64() + index*Marshal.SizeOf(typeof (EmotionData))),
                    data[index]);
            }
        }
        else {
    ...
    public class EmotionData {
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)] internal int[] reserved;
